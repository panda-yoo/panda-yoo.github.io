...
// Dynamically import all raw markdown posts
const postModules = import.meta.glob('/src/_posts/**/*.md', { query: '?raw', import: 'default', eager: true });

export default function Posts() {
  const location = useLocation();
  const { setScrollProgress } = useOutletContext<{ setScrollProgress: (p: number) => void }>();
  const pathParts = location.pathname.split('/posts/');
  let currentPath = (pathParts.length > 1 && pathParts[1]) ? pathParts[1] : 'blog/hello-world';
  
  const markdownKey = `/src/_posts/${currentPath}.md`;
  const rawMarkdown = (postModules[markdownKey] as string) || '# Post not found.\n\nThe requested post does not exist.';
  const markdownContent = preprocessObsidian(rawMarkdown);

  // Extract headings for Sidebar (H1, H2, H3)
  ...
  const h1Text = headings.find(h => h.level === 1)?.text || 'Untitled Post';
        ...
        <article className="p-8 lg:p-12 min-w-0 max-w-4xl mx-auto">
          <header className="mb-10">
            <nav className="text-xs text-theme-text-muted flex items-center mb-4">
              <Link to="/posts" className="hover:text-theme-text dark:hover:text-white transition-colors">Posts</Link>
        ...
        <section className="flex-1 px-8 pb-8">
          ...
          <ul className="flex flex-col w-full relative border-l border-zinc-200 dark:border-[#30363d]">
            ...
            {subheadings.length === 0 && (
              <li className="text-[13px] text-zinc-500 dark:text-[#8b949e] italic pl-4 py-2">No sections</li>
            )}
          </ul>
        </section>

        <div className="mt-auto p-6 border-t border-theme-border/50 bg-theme-sidebar">
          <Link to="/posts" className="flex items-center justify-between p-3 border border-theme-border/50 rounded-lg hover:bg-theme-sidebar-hover transition-colors group">
            <div className="flex flex-col">
              <span className="text-[10px] uppercase font-bold text-theme-text-muted tracking-wider mb-0.5">Next Post</span>
              <span className="text-[13px] font-medium text-theme-text group-hover:text-theme-accent transition-colors">All Posts</span>
            </div>
