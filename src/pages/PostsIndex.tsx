...
import { BookOpen, Search, Clock, Type, Folder as FolderIcon } from 'lucide-react';
...
const uniqueFolders = Array.from(new Set(allPosts.map(p => p.folder)));
...
export default function PostsIndex() {
  ...
  const filteredAndSortedPosts = useMemo(() => {
    return allPosts
      .filter(post => {
        if (folderFilter && post.folder !== folderFilter) return false;
        if (searchQuery) {
          const q = searchQuery.toLowerCase();
          return post.title.toLowerCase().includes(q) || post.description.toLowerCase().includes(q) || post.folder.toLowerCase().includes(q);
        }
        return true;
      })
      .sort((a, b) => {
      ...
      <div className="space-y-12">
        <div className="border-b border-theme-border/50 pb-12">
          ...
          <p className="text-[17px] text-theme-text-secondary dark:text-[#a1a1aa] font-medium max-w-2xl leading-relaxed">
            My blog posts, thoughts, and assorted articles.
          </p>
        </div>

        {/* Folders Section */}
        {!folderFilter && !searchQuery && uniqueFolders.length > 0 && (
          <div className="mb-12">
            <h2 className="text-[11px] font-semibold text-theme-text-muted dark:text-[#71717a] uppercase tracking-widest mb-6">Categories</h2>
            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-5">
              {uniqueFolders.map((folder, idx) => {
                const folderPosts = allPosts.filter(p => p.folder === folder);
                return (
                  <button 
                    key={idx} 
                    onClick={() => setSearchParams({ folder })}
                  ...
                  >
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center border border-emerald-500/10 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 transform group-hover:scale-110 transition-transform duration-300 shrink-0">
                      <FolderIcon className="w-5 h-5" strokeWidth={1.5} />
                    </div>
                    <div>
                      <h3 className="text-base font-semibold text-theme-text dark:text-white mb-1 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors uppercase tracking-tight">
                        {folder}
                      </h3>
                      <p className="text-[13px] text-theme-text-secondary dark:text-[#a1a1aa]">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-theme-border/50 pb-6">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-[11px] font-semibold text-theme-text-muted dark:text-[#71717a] uppercase tracking-widest">
              {folderFilter ? `${folderFilter} Posts` : 'All Posts'}
            </h2>
        ...
        <div className="grid sm:grid-cols-2 gap-5 pb-12">
          {filteredAndSortedPosts.map((post, idx) => (
            <Link key={idx} to={`/posts/${post.path}`} className="flex flex-col p-6 border border-theme-border/70 bg-theme-sidebar/30 hover:bg-theme-sidebar rounded-2xl group transition-all duration-300 hover:-translate-y-1 hover:shadow-sm hover:border-[var(--color-theme-border)]">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-[10px] font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 px-2 py-0.5 rounded-sm">
                  {post.folder}
                </span>
                {post.dateStr && (
              ...
              <h3 className="text-[17px] font-semibold text-theme-text dark:text-gray-100 mb-2 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors leading-snug">
                {post.title}
              </h3>
